...
```
3. 大管家会自动：主题确认 → 设计流程图 → **字段协商** → 创建报名表 + 实时通知 → 内容确认 → 发送群通知 → 作品初审 → 输出方案 → 生成海报
...
### 📋 内容二次确认（v1.1.0）
发送群通知前先展示预览，等发起人确认后再发。

### 🤖 bot 入群检查（v1.1.0）
发送通知前检查 bot 是否在目标群中，不在则引导手动拉入。
...
用户 @大管家 → 记录发起人
  ↓
0️⃣主题确认 → 2️⃣设计图 → 3️⃣信息采集（字段协商+实时通知+bot入群+内容确认）
  ↓                                  ↓
4️⃣验证/初审 ←──────────────────────── 5️⃣策划
  ↓
6️⃣汇总（通知发起人）
