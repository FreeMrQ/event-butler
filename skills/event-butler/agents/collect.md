...
飞书群消息**必须**使用 `lark-cli im +messages-send` 命令，**不能**使用内置 `send_message` 工具。

### 超链接格式

飞书 **不支持** Markdown `[文字](url)` 格式。必须使用 post 的 `a` 标签：

```json
{"tag":"a","text":"📋 点击填写报名表","href":"https://xxx.feishu.cn/share/base/form/shrcn..."}
```

### 海报 + 文字分两条发

图片和文字不能合并到一条消息。先发图片，再发文字：

```bash
# 1. 图片必须复制到当前目录（lark-cli 不接受绝对路径）
cp /tmp/poster.png ./poster.png
...
lark-cli im +messages-send \
  --chat-id <chat_id> \
  --msg-type post \
  --content '{"zh_cn":{"title":"标题","content":[[...]]}}' \
  --as bot
```

### ❌ 禁止的做法
- `send_message` 工具 — 不支持 `a` 标签超链接，链接会变成纯文本
- Markdown `[文字](url)` — 飞书不渲染为超链接
- `--image` 使用绝对路径 — lark-cli 只接受当前目录相对路径
             ...
             "btn_list": [
               {
                 "text": "查看报名表",
                 "btn_action": "openLink",
                 "link": [{"value_type": "text", "value": "<share链接>"}]
               }
             ]
           }
         }
...
> ⚠️ **关键参数：** `receiver` 中的 `id` 必须是发起人的 open_id（来自 Orchestrator 状态表），不可硬编码。`watched_field_name` 选用报名表中第一个必填字段名。工作流按钮链接后续更新为发布后的 share 链接。
...
1. 组装两条消息的完整内容预览：
   ...
   **消息 2 — 文字（post 格式，含可点击链接）：**
   ...
   ⚠️ **链接必须用 post `a` 标签：** `{"tag":"a","text":"📋 点击填写报名表","href":"https://xxx..."}`
   
   ❌ 禁止 Markdown `[文字](url)` — 飞书不渲染
   ❌ 禁止纯文本 URL — 飞书不自动转超链接
   ❌ 禁止 `send_message` 工具 — 不支持 `a` 标签

2. 展示预览给用户，附确认选项：
   ```
   📤 即将发送到「XXX」群（分两条：海报 + 文字），请确认：
...
3. **只有用户确认后，才执行发送。** 发送步骤：
   ```bash
   # 海报必须复制到当前目录
   cp /path/to/poster.png ./poster.png
   ...
   # 消息 2：文字（post + a 标签超链接）
   lark-cli im +messages-send \
     --chat-id <chat_id> \
     --msg-type post \
     --content '{"zh_cn":{"title":"🏀 第二届容知日新篮球赛 — 报名开启！","content":[[{"tag":"text","text":"赛制：3V3 半场 | 8 支队伍 | 每队 5 人 | 联赛制"}],[{"tag":"text","text":"场地：公司球场"}],[{"tag":"text","text":"报名截止：赛前 1 周"}],[{"tag":"text","text":""}],[{"tag":"a","text":"📋 点击填写报名表","href":"https://xxx.feishu.cn/share/base/form/shrcn..."}],[{"tag":"text","text":""}],[{"tag":"text","text":"💡 每队只需队长填写一次，提交后自动记录"}]]}}' \
     --as bot
   ```

4. 发送成功后，更新工作流按钮链接为 share 链接：
   ```
   lark-cli base +workflow-update --base-token <token> --workflow-id <id> --json '...' --as user
   ```
...
- `lark-cli base +workflow-create` / `+workflow-enable` / `+workflow-update`（实时报名通知 — 创建后自动执行）
- `lark-cli im +chat-search`（搜索目标群）
- `lark-cli im chat.members get`（检查 bot 是否在群中）
- `lark-cli im +messages-send --image`（发送海报 — 文件须在 cwd）
- `lark-cli im +messages-send --msg-type post --content`（发送文字 + a 标签超链接）
